(function (global, factory) {
	...
	var GestureHandling = LA3.Handler.extend({
		...
		addHooks: function () {
			this._handleTouch = LA3.bind(this._handleTouch, this);

			this._setGestureHandlingOptions();
			this._disableInteractions();

			//Uses native event listeners instead of LA3.DomEvent due to issues with Android touch events turning into pointer events
			...
			LA3.DomEvent.on(this._map._container, "wheel", this._handleScroll, this);
			LA3.DomEvent.on(this._map._container, "mouseenter", this._handleMouseOver, this);
			LA3.DomEvent.on(this._map._container, "mouseleave", this._handleMouseOut, this);

			// Listen to these events so will not disable dragging if the user moves the mouse out the boundary of the map container whilst actively dragging the map.
			LA3.DomEvent.on(this._map, "movestart", this._handleDragging, this);
			LA3.DomEvent.on(this._map, "move", this._handleDragging, this);
			LA3.DomEvent.on(this._map, "moveend", this._handleDragging, this);
			...
			LA3.DomEvent.off(this._map, "enterFullscreen", this._onEnterFullscreen, this);
			LA3.DomEvent.off(this._map, "exitFullscreen", this._onExitFullscreen, this);
			LA3.DomEvent.on(this._map, "enterFullscreen", this._onEnterFullscreen, this);
			LA3.DomEvent.on(this._map, "exitFullscreen", this._onExitFullscreen, this);

			LA3.DomUtil.addClass(this._map._container, "leaflet-gesture-handling");
		},
		...
		removeHooks: function () {
			...
			LA3.DomEvent.off(this._map._container, "wheel", this._handleScroll, this);
			LA3.DomEvent.off(this._map._container, "mouseenter", this._handleMouseOver, this);
			LA3.DomEvent.off(this._map._container, "mouseleave", this._handleMouseOut, this);

			LA3.DomEvent.off(this._map, "movestart", this._handleDragging, this);
			LA3.DomEvent.off(this._map, "move", this._handleDragging, this);
			LA3.DomEvent.off(this._map, "moveend", this._handleDragging, this);

			this._map.off("popupopen", this._handleScrollOnPopup, this);
			this._map.off("popupclose", this._handleScrollOnPopup, this);

			LA3.DomUtil.removeClass(this._map._container, "leaflet-gesture-handling");
		},
		...
		_enableWarning: function (gesture) {
			...
			LA3.DomUtil.addClass(this._map._container, "leaflet-gesture-handling-" + gesture);
			LA3.DomUtil.addClass(this._map._container, "leaflet-gesture-handling-warning");
		},

		_disableWarning: function (gesture, delay) {
			clearTimeout(this._isFading);
			this._isFading = setTimeout(
				LA3.bind(function (gesture) {
					LA3.DomUtil.removeClass(this._map._container, "leaflet-gesture-handling-" + gesture);
				}, this, gesture),
				delay || this._map.options.gestureHandlingOptions.duration
			);
			LA3.DomUtil.removeClass(this._map._container, "leaflet-gesture-handling-warning");
		},
		...
		_parseGestureHandlingOptions: function () {
			...
			var options = LA3.extend(this._map.options.gestureHandlingOptions, gestureHandlingOptions);
		...
		_handleTouch: function (e) {
			...
			var ignore = LA3.DomUtil.hasClass(e.target, "leaflet-interactive") || e.target.closest('.leaflet-control-container') || e.target.closest('.leaflet-popup-pane');

			if (ignore) {
				if (LA3.DomUtil.hasClass(e.target, "leaflet-interactive") && e.type === "touchmove" && e.touches.length === 1) {
		...
		_handleScrollOnPopup: function (e) {
			LA3.DomEvent[e.type == 'popupopen' ? 'on' : 'off']
	...
	LA3.Map.mergeOptions({
		gestureHandlingOptions: gestureHandlingOptions
	});

	LA3.Map.addInitHook("addHandler", "gestureHandling", GestureHandling);
